...
export function isValidSlide(tileIdx: number, emptyIdx: number, gridSize: number): boolean {
...
export function getMovableTileIndices(emptyIdx: number, gridSize: number): number[] {
  const emptyRow = Math.floor(emptyIdx / gridSize);
  const emptyCol = emptyIdx % gridSize;
  const valid: number[] = [];

  if (emptyRow > 0) valid.push((emptyRow - 1) * gridSize + emptyCol); // UP
  if (emptyRow < gridSize - 1) valid.push((emptyRow + 1) * gridSize + emptyCol); // DOWN
  if (emptyCol > 0) valid.push(emptyRow * gridSize + (emptyCol - 1)); // LEFT
  if (emptyCol < gridSize - 1) valid.push(emptyRow * gridSize + (emptyCol + 1)); // RIGHT
...
export function generateSolvableBoard(level: GameLevelConfig, shuffleSteps = 60): TileItem[] {
  ...
  let lastMovedIdx = -1;
  const steps = Math.max(shuffleSteps, level.gridSize * 15);

  for (let i = 0; i < steps; i++) {
    const movables = getMovableTileIndices(emptyIdx, level.gridSize).filter((idx) => idx !== lastMovedIdx);
...
export function parseBoardString(boardJson: string, level: GameLevelConfig): TileItem[] {
  try {
    ...
    return values.map((val, idx) => ({
      id: `tile-mp-${idx}-${val}`,
      value: val,
      correctIndex: val > 0 ? val - 1 : level.gridSize * level.gridSize - 1,
